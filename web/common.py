...
def format_date(t, when) -> str:
    """«1 de octubre» / «October 1». Las fechas se guardan en UTC y se muestran así, sin hora."""
    return t("date_fmt", d=when.day, m=t(f"mon_{when.month}")) if when else ""
...
def render(request: Request, name: str, status_code: int = 200, headers: dict | None = None, **ctx) -> HTMLResponse:
    ...
    response = templates.TemplateResponse(request, name, {
        "t": t, "lang": lang, "theme": theme, "css_tokens": css_tokens(),
        ...
        "format_duration": format_duration, "format_date": lambda when: format_date(t, when),
        "user": getattr(request.state, "user", None), "csrf_token": getattr(request.state, "csrf", ""),
