...
Manual de trabajo del repositorio. Lo leen Cursor, Claude Code y cualquier otro asistente:
no depende de una herramienta. `CLAUDE.md` y `.cursor/rules/` apuntan acá.
...
## Cómo trabajar la interfaz
- **Usar el ancho, sin compactar** (BRAND.md §6): campos y datos relacionados lado a lado cuando el ancho lo permite, apilados en móvil. Evitar el sobrescroll; mantener espaciados y tamaños del manual.
- **Pantalla pulida de entrada** (BRAND.md §6 «Movimiento e interacción»): transiciones suaves sin parpadeos, paneles animados, estados de carga en cada acción, tooltips para ayudas secundarias, y estado consistente — lo que se ve es lo guardado, con autoguardado, sin perder nada al cambiar idioma o tema, recargar o volver atrás.
- Antes de dar una pantalla por terminada: cambio de idioma y de tema a mitad de un formulario, recarga, botón atrás, doble clic, sesión vencida, móvil y alto de laptop (1366×657).
...
- PRs a `main` siempre con **rebase and merge** (`gh pr merge --rebase --delete-branch`): historial lineal, sin merge commits, y la rama se borra local y remota al mergear.
